...
const HEALTH_SYMPTOMS: Record<string, HealthSymptomInfo> = {
    ...
    fever: {
        severity: "high",
        advice: "La fièvre est un signe que le corps combat une infection. Une température normale pour un chien est de 38-39°C, et pour un chat 38-39.2°C. Au-dessus de 39.5°C, consultez un vétérinaire rapidement.",
        redFlags: [
            "Température > 40°C",
            "Léthargie sévère",
            "Refus de boire",
    ...
    cough: {
        severity: "medium",
        advice: "La toux peut être causée par diverses raisons : irritation, infection, allergie, ou problème cardiaque. Une toux occasionnelle n'est pas alarmante, mais une toux persistante nécessite une consultation vétérinaire.",
        redFlags: [
            "Toux avec sang",
            "Difficulté respiratoire",
            "Toux nocturne",
    ...
    limping: {
        severity: "medium",
        advice: "La boiterie peut résulter d'une blessure, arthrite, ou problème articulaire. Si la boiterie est soudaine et sévère, consultez rapidement. Pour une boiterie légère, surveillez 24-48h.",
        redFlags: [
...
export function getHealthAdvice(symptom: string): AdviceData {
    ...
    if (!info) {
        return {
            ...
            answer: "Pour tout problème de santé, nous recommandons vivement de consulter un vétérinaire. Seul un professionnel peut établir un diagnostic précis et prescrire un traitement approprié.",
            severity: "unknown",
            confidence: 50,
            nextSteps: [
 ...
 */
export function isFoodToxic(food: string, species: "dog" | "cat"): boolean {
    const toxicFoods = getToxicFoods(species);
    const foodLower = food.toLowerCase();
    return toxicFoods.some((toxic) => foodLower.includes(toxic.toLowerCase()));
}
