...
const PetDietCard: React.FC<PetDietCardProps> = ({
    petId,
    petName,
    petSpecies,
    onClose,
    ...
    useEffect(() => {
        feedingService
            ...
            .then((data: any) => {
                ...
                if (isGrouped) {
                    const petGroup = data.find(
                        (g: any) => Number(g.pet_id) === Number(petId),
                    );
                    ...
                    if (data[0]?.pet?.breed) {
                        const match = data.find(
                            (s: any) => Number(s.pet_id) === Number(petId),
                        );
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400">
                                {breed ||
                                    (petSpecies === "dog" ? "Chien" : "Chat")}
                                {!isLoading &&
                                    ` · ${schedules.length} repas/jour`}
                            </p>
                        </div>
                    </div>
                    <button
                            ...
                            >
                                ...
                                <div className="flex-1">
                                    <p className="font-semibold text-gray-900 dark:text-white">
                                        {SLOT_LABELS[schedule.meal_slot] ||
                                            schedule.meal_slot}
                                    </p>
