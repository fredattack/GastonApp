...
const AIMessageCard: React.FC<AIMessageCardProps> = ({
    ...
    const getPetInfo = () => {
        if (!attachedEvent.pets || attachedEvent.pets.length === 0) {
            return [];
        }
    ...
    const primaryData = extractPrimaryData(attachedEvent);
    const petInfo = getPetInfo();

    // Get requestType from AI response or default to createEvent
